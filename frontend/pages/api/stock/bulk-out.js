  ...
  try {
    ...
    for (const row of csvData) {
      try {
        ...
        if (previousQuantity < row.quantity) {
          results.errors.push(`${row.sku}: Insufficient stock (available: ${previousQuantity}, requested: ${row.quantity})`)
          continue
        }
    ...
    res.status(200).json({
      success: true,
      message: `Processed ${results.processed} stock-out entries`,
    ...
    if (error.message === 'Authentication failed') {
      ...
      res.status(500).json({ 
        message: 'Failed to process bulk stock-out',
        error: error.message 
      })
    }
  }
